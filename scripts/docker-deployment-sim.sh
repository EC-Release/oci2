...
docker run --rm --name server-tls --env-file=test/server+tls.env enterpriseconnect/plugins:v1 -ver
printf "\n\n\n*** test client+vln v1 plugin w/ docker\n\n"
docker run --rm --name client-vln --env-file=test/client+vln.env enterpriseconnect/plugins:v1 -ver
